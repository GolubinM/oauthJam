...
function getProducts() {
  ...
  let arrProducts = [], formData, options;
  ...
  getRequestOptions();
  let response, arrCardsObj, cursorCardsTotal = 100, arrCards = [], responsePage = 1
...
function getAdvStatistic() {
  ...
  campIds = ApiUtils.readRangeSS(SSId, `${SheetNames.advList}!A2:D`, "UNFORMATTED_VALUE", "SERIAL_NUMBER")
  ...
  let attempt = 0, maxAttempt = 3, gapTime = 65e3, table = [], startTime, waitFor = 0, xRatelimitRetry, xRatelimitRemaining = 1;
  let successRes = [], idReq = 0;
  for (let request of requests) {
    if (!xRatelimitRemaining && waitFor > 0) { Utilities.sleep(waitFor) }
  ...
  if (attempt === maxAttempt) { console.log(`Запрос ${idReq} не был обработан корректно.`) }
  ...
  function getWbResponses(request, idReq) {
    ...
    console.log(`Запрос ${idReq}: code ${respCode}, xRatelimitRemaining:${xRatelimitRemaining} ${(msg = respCode == 400 ? result.value?.content : "")}.`)
    if (msg) { inform(SS, [msg, "Статистика рекламы"]) }
    if (respCode === 429) {
  ...
  function parseAndSaveResp(successRes) {
    table = successRes.reduce((concTab, rsp_tabData) => {
      ...
      if (out.length) { concTab = concTab.concat(out) }
      return concTab
    }, [])
    if (table.length) {
      console.log("Ответ получен. Парсим.")
      //   console.log("Группируем полученные записи")
      ...
      if (addRows > 0) { table = SheetUtils.clearByAddBlankRow(table, addRows) }
      let res = ApiUtils.writeToSS(SSId, table, `${SheetNames.advStat}!A${rowUpdateFrom}`)
      if (res) { msg += "Обновление статистики рекламных кампаний завершено. Обновлено строк: " + tableLength }
    } else { msg += "Данные для обновления не были получены." }
  }
}
...
function updateHistory() {
  ...
  if (addClearRows > 0) { data = SheetUtils.clearByAddBlankRow(data, addClearRows) }
...
function getCampHistory(startDate, endDate) {
  ...
  catch (e) { console.log("ошибка загрузки данных: ", e.stack) }
}
function getAdvCampList() {
  ...
  try { ss.getFilter().remove() } catch (ex) { }
  ...
  try {
    ...
    if (data.getResponseCode() === 200) {
      ...
      let oldDataObj = oldData.reduce((res, row) => { res[row[0]] = row; return res }, {});
      ...
      if (rowsToClean > 0) { updatedData = SheetUtils.clearByAddBlankRow(updatedData, rowsToClean) }
...
function statKeyWords(proceedFromPage) {
  ...
  let { maxDateStatAB, minDateStatAB } = statAB.reduce((res, elm) => {
  ...
  console.log(`Загрузка данных с ${DateUtils.getStrDateFromSerialNumberDate(dateFromSer)} по ${DateUtils.getStrDateFromSerialNumberDate(dateToSer)}`);
  ...
  campIds.forEach((row) => { if (row[1] == 8) { campIdsSet.add(row[0]); } });
  let stepDays = 6,
  ...
  if (proceedFromPage) {
    console.log(`Продолжаем сбор с request:${proceedFromPage * maxRequestsPerLoad}`);  //продолжаем со страницы proceedFromPage
  } else {
    console.log(`Загрузка с начала списка. Максимальное кол-во запросов на сессию(~5мин.): ${maxRequestsPerLoad}`);
  }
  ...
  let attempt = 0, maxAttempt = 6;
  let successRes = [], counterPage = 0, totalReq = 0;
  for (let request of flowRequsets) {
    let responseCode = null;
    counterPage++;
    if (!(counterPage % 30)) { console.log("requests;", counterPage) };
    while (attempt < maxAttempt) {
      try {
    ...
    if (attempt === maxAttempt) { console.warn(`Ответ на запрос ${counterPage} не был получен. Код:`, responseCode) }
  }

  // вызов функции обработки и сохранения данных
  proceedFromPage++;
  let isFinish = false, msg, propValue, setTriggFlag;
  if (requestsLengthAll > proceedFromPage * maxRequestsPerLoad) {
    console.log("Устанавливаем триггер на следующий запуск скрипта через 1 минуту. Сбор со страницы:", proceedFromPage * maxRequestsPerLoad);
    ...
    if (isFinish) {
      ...
      if (Object.keys(aggregatedObjs).length) {
        newData = statAB.map((row, ind) => {
          if (flowIndRows.has(ind)) {     //  если номер строки вкходит в набором обновляемых строк
            let key = `${row[1]}~${row[0]}`;
...
function analyticNmIdPeriod(propValue) {
  ...
  const headers = { Authorization: getkey("rekl") };
  let successResults = [], idReq = 0;
  console.log(`Загрузка данных  с ${DateUtils.getStrDateFromSerialNumberDate(dateFromSer)} по ${DateUtils.getStrDateFromSerialNumberDate(dateToSer)}`);
  let nmIds = ApiUtils.readRangeSS(SSId, `${SheetNames.products}!A2:C`, "UNFORMATTED_VALUE", "SERIAL_NUMBER");
  nmIds = nmIds.reduce((res, row) => { if (row[0] && row[2]) { res.push(row[0]) }; return res }, []); // [nmId1,nmId2..]
  ...
  let attempt, maxAttempt = 3, gapTime = 21e3, table = [], startTime, waitFor = 0, xRatelimitRetry, xRatelimitRemaining = 1;
  ...
  if (totalRequstCount) {
    ...
    if (propValue) { `Сбор данных будет продолжен с запроса:${propValue * maxRequests}` }
    ...
    if (attempt === maxAttempt) { console.log(`Запрос ${idReq} не был обработан корректно. Превышен лимит запросов.`) }
  ...
  function getWbResponses(request, idReq) {
    ...
    let headers = response.getHeaders()
    xRatelimitRemaining = +(headers['x-ratelimit-remaining'] || 0);
    console.log(`Запрос ${idReq}: code ${respCode}, xRatelimitRemaining:${xRatelimitRemaining} ${(message = respCode == 400 ? response.getContentText() : "")}.`)
  ...
  function parseAndSaveResp(successResults) {
    ...
    if (table.length) {
      ...
      if (addClearRows > 0) { table = SheetUtils.clearByAddBlankRow(table, addClearRows); }
...
function singleKeyWords() {
  ...
  if (campIds.length > maxRequestsPerLoad) {
    inform(SS, [`Кол-во отобранных кампаний > ${maxRequestsPerLoad}. Список будет ограничен последними ${maxRequestsPerLoad} кампаниями.`, "keyWords"])
  ...
  let attempt = 0,
    maxAttempt = 6;
  let successRes = [], counterPage = 0;
  for (let request of flowRequsets) {
    let responseCode = null
    counterPage++;
    if (!(counterPage % 33)) { console.log("requests;", counterPage) };
  ...
  function parseAndSaveResp(successRes) {
    ...
    if (allData.length) {
      ...
      if (rowsToClear > 0) { allData = SheetUtils.clearByAddBlankRow(allData, rowsToClear) };
      allData.unshift(header);
      let res = ApiUtils.writeToSS(SS.getId(), allData, `${SheetNames.keyWords}!A5`);
      if (res) { message += "Все данные получены. Сбор завершен." };
    } else {
      message += "Данные для обновления не были получены.";
    }
 ...
 */
function analyticsJam() { analyticsJam(reportType = "DETAIL_HISTORY_REPORT") }
function searchTextJam() { analyticsJam(reportType = "SEARCH_QUERIES_PREMIUM_REPORT_TEXT") }
function analyticsJam(reportType = "SEARCH_QUERIES_PREMIUM_REPORT_TEXT") {
  ...
  let SSId = SS.getId();
  let headers = { 'Authorization': getkey("analytics") }, sheetName;
  if (reportType === "DETAIL_HISTORY_REPORT") { sheetName = analyticsJam }
  else if (reportType === "SEARCH_QUERIES_PREMIUM_REPORT_TEXT") { sheetName = SheetNames.searchJam }
  ...
  if (filteredNmIDs.length < 1000 && filteredNmIDs.length) { nmIDs = filteredNmIDs.map(row => row[0]) } else { nmIDs = [] }
  let payload
  ...
  function tabToObjectHeaders(table, reportType) {
    ...
    let fixedKeys
    if (reportType === "DETAIL_HISTORY_REPORT") {
      fixedKeys = ['nmID', 'dt', 'openCardCount', 'addToCartCount', 'ordersCount', 'ordersSumRub', 'buyoutsCount', 'buyoutsSumRub', 'cancelCount', 'cancelSumRub', 'addToCartConversion', 'cartToOrderConversion', 'buyoutPercent']
    } else if (reportType === "SEARCH_QUERIES_PREMIUM_REPORT_TEXT") {
      fixedKeys = ['Text', 'NmID', 'SubjectName', 'BrandName', 'VendorCode', 'Name', 'Rating', 'FeedbackRating', 'MinPrice', 'MaxPrice', 'Frequency', 'MedianPosition', 'AveragePosition', 'OpenCard', 'OpenCardPercentile', 'AddToCart', 'AddToCartPercentile', 'OpenToCart', 'OpenToCartPercentile', 'Orders', 'OrdersPercentile', 'CartToOrder', 'CartToOrderPercentile', 'Visibility']
    }
...
function searchTextsJam(propValue) {
  ...
  nmIdsTable = nmIdsTable.reduce((res, row) => { if (row[0] && row[2]) { res.push(row[0]) }; return res }, []); // [nmId1,nmId2..]
  ...
  if (totalRequstCount) {
    let requests = dates.map(datesStr => {
      ...
      payload.currentPeriod = { "start": datesStr.currentPeriod, "end": datesStr.currentPeriod };
      return {
    ...
    propValue = +(propValue || 0);
    console.log("propValue:", propValue);
    if (propValue) { console.log(`Сбор данных будет продолжен с запроса:${propValue * maxRequests}`) }
    requests = requests.slice(propValue * maxRequests, (propValue + 1) * maxRequests)
    console.log(`Отобрано запросов:${requests.length}. Максимум запросов за 5 мин: 15.`);
  ...
  function parseAndSaveResp(successResults) {
    table = successResults.reduce((concTab, respData) => {
      let out, currentDate = Math.floor(respData.date);
    ...
    if (table.length) {
      ...
      table.sort((a, b) => a[2] - b[2]).sort((a, b) => `${a[1]}`.localeCompare(`${b[1]}`)).sort((a, b) => a[0] - b[0]);
